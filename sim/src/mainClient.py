# Server version of simulator
# TODO: convert sunlight to be a socket server like this
...
HOST = ''       #listens to every interface
    ...
    while True:
        try:
            for i in CAN_Test_Data: 
                conn.send(bytearray(eth_header_CAN + i[3::-1] + i[7:3:-1] + i[16:7:-1]))
            logging.debug("CAN sent.")
            conn.sendall(bytearray(eth_header_GPS) + GPS_Test_Data.encode())
            logging.debug("GPS sent.")
            for i in IMU_Test_Data:
                conn.sendall(bytearray(eth_header_IMU) + i)
