...
async def GPSparse(array):
    hr = int.from_bytes(array[0:2], "big")
    min = int.from_bytes(array[2:4], "big")
    ms = int.from_bytes(array[4:7], "big")
    latitude_Deg = int.from_bytes(array[7:9], "big")
    latitude_Min = int.from_bytes(array[9:15], "big")
    NorthSouth = int.from_bytes(array[15:16], "big")
    longitude_Deg = int.from_bytes(array[16:19], "big")
    longitude_Min = int.from_bytes(array[19:25], "big")
    EastWest = int.from_bytes(array[25:26], "big")
    speedInKnots = int.from_bytes(array[26:30], "big")
    day = int.from_bytes(array[30:32], "big")
    month = int.from_bytes(array[32:34], "big")
    year = int.from_bytes(array[34:38], "big")
    magneticVariation_Deg = int.from_bytes(array[38:42], "big")
    magneticVariation_EastWest = int.from_bytes(array[42:43], "big")
    print(f"hr: {hr}")
    print(f"min: {min}")
    print(f"ms: {ms}")
