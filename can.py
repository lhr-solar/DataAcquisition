...
def CANparse(canArray):
    ...
    print("Payload: " + canArray + "\n")
    print(CANIDs[canID], str(index), rawData) #to test on the local computer

    '''
    str1 = ''.join((str(e) for e in canArray))
    f = open("testCAN.txt", "a")
    f.write("ACTUAL DATA: " + str1 + "\n")
    f.write("canID: " + str(canID) + "\n")
    f.write("index: " + str(index) + "\n")
    f.write("rawData: " + str(rawData) + "\n")
    f.close()
    '''
